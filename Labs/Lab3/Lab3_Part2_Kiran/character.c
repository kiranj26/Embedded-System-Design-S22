                       ...
                       int * free_buffer_count, int * filled_buffer_count,int *storage_character_count, int * total_characters_count, int buffer0_size)
{
    ...
    if(cmd_detected)            // if command detected
    {
        ...
        case '+':               // if input character is '+'
            ...
            buff_not_cleared[(*new_buffer_index)] = 1;                      // set flag to indicate new buffer is created

            // printf("\033[1;0mNew buffer index = %d\n\r",(*new_buffer_index)); // print new buffer index
            ...
            if (buffer_to_delete == 0)
            {
                ...
                if(new_buffer_arr[buffer_to_delete] == NULL)
                {
        ...
        case '?':  // if input character is '?'
            ...
            {
                if( (new_buffer_arr[i] == NULL) || (new_buffer_arr[i] == 0) )                   // check if the current buffer is already cleared
                {
            ...
            *total_characters_count = 0;
            ...
            printf_tiny(" \033[1;34m|************ REPORT ENDED  *******************|\n\r\n\r\n\r");
